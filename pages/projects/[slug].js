...
export default function ProjectPage() {
  ...
  if (slug === "charon") {
    ...
    detailedDescription = "Funds land in a secure wallet merchants control, with a clear ledger view of incoming and outgoing transactions. When ready to cash out, users can send funds to a presaved offramp address. Funds can be swapped or sent to a bank account with minimal steps. Wallet info is also easily accessible from the POS app.\n\nThe wallet itself is a standalone web app. Log in via email (with optional 2FA), view balances, send to any address, or swap between supported digital currencies. No blockchain jargon, just practical tools for managing funds.\n\n<strong>Role:</strong> Project Manager / Founder\n<strong>Timeline:</strong> May 2025\n<strong>Tools Used:</strong> ClickUp, Jira, Notion\n<strong>Stack:</strong> Privy, React (frontend), Node.js & TypeScript (backend)\n<strong>Contributions:</strong> Defined product requirements, managed implementation milestones, and created internal documentation\n\nManaged product delivery using ClickUp and Jira to track tasks across engineering and design. Documented wallet flows, and oversaw addition of login, 2FA, and send/receive functionality using Privy infrastructure."
    images = [
